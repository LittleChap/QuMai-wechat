<template>
  <section>
    <div class="flexBox center" @click="setCurrentSelect">
      <i class="icon-select v-single-select"
         :class="{'icon-change-highlight': currentSelect===item.addressId, 'icon-change-disable': currentSelect!==item.addressId }"
      ></i>
      <div class="flexChild">
        <p class="flexBox">
          <span class="flexChild">联系人：{{item.name}}</span>
          <span class="flexChild">{{item.mobile}}</span>
        </p>
        <p>收货地址：{{item.province}}&nbsp;{{item.city}}&nbsp;{{item.region}}&nbsp;{{item.address}}</p>
      </div>
    </div>
    <div class="flexBox center bottomBox">
...
<script type="text/ecmascript-6">
  export default {
    ...
    computed: {
      currentSelect: {
        get () {
          if (this.$store.getters.addressInfo && this.$store.getters.addressInfo.addressId > -1) {
            return this.$store.getters.addressInfo.addressId
          } else {
            return 0
          }
        },
        set (newValue) {
          this.$store.getters.addressInfo.addressId = newValue
        }
      }
    },
    methods: {
      // 设置当前选中的地址
      setCurrentSelect () {
        this.currentSelect = this.item.addressId
        sessionStorage.setItem('currentAddressId', this.item.addressId)
        this.$router.go(-1)
      },
      select () {
