...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    async created () {
      ...
      const res = await fetch(url, {goodsId: this.$route.params.id, type: 3})
      if (res.code == 1) {
    ...
    methods: {
      ...
      async getLabelComments () {
        if (this.selectId == 0) {
          ...
          const url = '/mobileOrderItemComment/getOrderItemCommentList.do'
          const res = await fetch(url, {goodsId: this.$route.params.id, type: 3, labelId: this.selectId})
          if (res.code == 1) {
            this.evaluaItems = res.data.orderItemCommentArray
          }
        }
